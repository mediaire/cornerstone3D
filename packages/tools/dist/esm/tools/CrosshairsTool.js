...
class CrosshairsTool extends AnnotationTool {
    constructor(toolProps = {}, defaultToolProps = {
        ...
        this._computeToolCenter = (viewportsInfo) => {
            ...
            const { normal: normal1, point: point1, orientation: orientation1, } = this.initializeViewport(firstViewport);
            const { normal: normal2, point: point2, orientation: orientation2, } = this.initializeViewport(secondViewport);
            let normal3 = [0, 0, 0];
            let point3 = vec3.create();
            let orientation3 = Enums.OrientationAxis.REFORMAT;
            if (thirdViewport) {
                ({
                    normal: normal3,
                    point: point3,
                ...
                ], [normal1, normal2, normal3], [orientation1, orientation2, orientation3]);
            }
            else {
                vec3.add(point3, point1, point2);
                vec3.scale(point3, point3, 0.5);
                vec3.cross(normal3, normal1, normal2);
                this._computeCrosshairApplication([firstViewport.viewportId, secondViewport.viewportId], [normal1, normal2], [orientation1, orientation2]);
            }
        ...
        this.renderAnnotation = (enabledElement, svgDrawingHelper) => {
            ...
            if (this.viewportCrosshairsConfig[viewport.id] ===
                CrosshairsType.SAME_ORIENTATION) {
                let lineUID = 1;
                ...
                const pos = typeof this.toolWorldFocus === 'undefined'
                    ? [viewport.canvas.clientWidth / 2, viewport.canvas.clientHeight / 2]
                    : viewport.worldToCanvas(this.toolWorldFocus);
                drawLineSvg(svgDrawingHelper, annotationUID, `Line${lineUID++}`, [0, pos[1]], [pos[0] - crossHairRadius, pos[1]], {
                    color,
                    lineWidth,
                });
                drawLineSvg(svgDrawingHelper, annotationUID, `Line${lineUID++}`, [pos[0] + crossHairRadius, pos[1]], [viewport.sWidth, pos[1]], {
                    color,
                    lineWidth,
                });
                drawLineSvg(svgDrawingHelper, annotationUID, `Line${lineUID++}`, [pos[0], 0], [pos[0], pos[1] - crossHairRadius], {
                    color,
                    lineWidth,
                });
                drawLineSvg(svgDrawingHelper, annotationUID, `Line${lineUID++}`, [pos[0], pos[1] + crossHairRadius], [pos[0], viewport.sHeight], {
                    color,
                    lineWidth,
                });
            }
        ...
        this._dragCallback = (evt) => {
            ...
            if (this.viewportCrosshairsConfig[viewport.id] ===
                ...
                const newCenter = [0, 0, this.toolCenter[2]];
